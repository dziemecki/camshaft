...
try {
    $conn = new PDO("$dbadapter:host=$servername;dbname=$dbname", $username, $password);
    // set the PDO error mode to exception
    $conn->setAttribute(PDO::ATTR_ERRMODE, features::ERRMODE_EXCEPTION);
	$s_sql = "CREATE TABLE IF NOT EXISTS features( ".
    ...
    // use exec() because no results are returned
    $conn->exec($s_sql);
    if($newTable){echo "<br>Table 'features' created successfully</br>";}else{echo "<br>Table 'features' not changed</br>";}
    }
catch(PDOException $e)
    {
    echo $s_sql . "<br>Error for features table:" . $e->getMessage();
    }
